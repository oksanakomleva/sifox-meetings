  ...
  return (
    <div className="card" style={{ padding: 'var(--space-5)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-4)' }}>
        <button onClick={prevMonth} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 'var(--space-1)', color: 'var(--color-text-secondary)', lineHeight: 1 }}>‹</button>
        <span style={{ fontWeight: 'var(--font-weight-semibold)', fontSize: 'var(--font-size-base)' }}>
          {MONTHS_RU[month]} {year}
        </span>
        <button onClick={nextMonth} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 'var(--space-1)', color: 'var(--color-text-secondary)', lineHeight: 1 }}>›</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 4 }}>
        {WEEKDAYS.map(d => (
          <div key={d} style={{ textAlign: 'center', fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', fontWeight: 600, padding: '2px 0' }}>{d}</div>
        ))}
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isSelected ? 'var(--color-accent-6)' : isToday ? 'var(--color-surface-2)' : 'transparent',
...
// ── Meetings view (shared by "Мои встречи" and admin "Все встречи") ─────────────
...
export default function MeetingsView({ title, subtitle, fetchDone, fetchUpcoming, admin, onReanalyze }: Props) {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([fetchDone(), fetchUpcoming()])
      .then(([done, up]) => {
        setDoneMeetings(done.filter(m => doneStatuses.includes(m.status)))
          ...
          {(['done', 'upcoming'] as const).map(t => (
            <button
              ...
              style={{
                ...
                padding: 'var(--space-3) var(--space-5)', fontSize: 'var(--font-size-sm)',
          ...
          <>
            <div style={{ marginBottom: 'var(--space-4)', position: 'relative' }}>
              <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"
                style={{ width: 16, height: 16, position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-muted)', pointerEvents: 'none' }}>
                <circle cx="8.5" cy="8.5" r="5.5"/><path d="M13.5 13.5l3 3"/>
              </svg>
              <input className="input" value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Поиск по названию, тегам, содержанию…" style={{ paddingLeft: 36 }} />
            </div>
            ...
            {filteredDone.length === 0 ? (
              <div className="empty-state">
                ...
                <div className="empty-state-title">{search ? 'Ничего не найдено' : 'Нет записей встреч'}</div>
                <p className="empty-state-text">
                  {search ? 'Попробуйте другой запрос' : 'Когда запись будет готова, она появится здесь автоматически.'}
                </p>
              </div>
            ) : (
            ...
            <div>
              {selectedDay && (
                <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)', marginBottom: 'var(--space-3)' }}>
                  ...
                  <button onClick={() => setSelectedDay(null)} style={{ marginLeft: 8, color: 'var(--color-accent)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 'var(--font-size-xs)' }}>сбросить</button>
                </div>
              )}
              {filteredUpcoming.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-state-icon">📆</div>
                  <div className="empty-state-title">{selectedDay ? 'Нет встреч в этот день' : 'Нет запланированных встреч'}</div>
                  <p className="empty-state-text">Встречи из Google Календаря появятся здесь автоматически.</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 'var(--space-4)' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            marginBottom: 'var(--space-1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
          }}>
            {m.title || m.topic || 'Без названия'}
          </div>
          <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>
          ...
          {m.status === 'error' && m.error_message && (
            <div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-error)', marginTop: 4 }}>
              {m.error_message.slice(0, 80)}
            </div>
          )}
